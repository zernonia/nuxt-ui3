...
import type { ComboboxRootProps, ComboboxRootEmits, ComboboxItemProps } from 'reka-ui'
...
export interface CommandPaletteProps<G, T> extends Pick<ComboboxRootProps, 'multiple' | 'disabled' | 'modelValue' | 'defaultValue' | 'resetSearchTermOnBlur'>, Pick<UseComponentIconsProps, 'loading' | 'loadingIcon'> {
  /**
...
const rootProps = useForwardPropsEmits(reactivePick(props, 'as', 'disabled', 'multiple', 'modelValue', 'defaultValue', 'resetSearchTermOnBlur'), emits)
...
<template>
  <ComboboxRoot v-bind="rootProps" open :class="ui.root({ class: props.class })" ignore-filter>
    ...
    <ComboboxPortal disabled>
      <ComboboxContent :class="ui.content({ class: props.ui?.content })" :dismissable="false">
        <ComboboxEmpty v-if="!groups.length" :class="ui.empty({ class: props.ui?.empty })">
